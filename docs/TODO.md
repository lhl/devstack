...
|-----------|------|--------|
| [`@vanillagreen/pi-background-tasks`](https://www.npmjs.com/package/@vanillagreen/pi-background-tasks) | Explicit `bg_task` / `/bg` shell tasks, auto-backgrounded monitors, completion + output-pattern wakeups | Isolated `pi -e npm:@vanillagreen/pi-background-tasks@1.6.0` smoke test passed; next: full-stack/interactive wake tests |
...
5. ⏳ Verify LLM-tool `bg_task` invocation with `notifyOnOutput` + `notifyPattern` for a `READY` / `BG_DONE` line.
6. ⏳ Verify auto-backgrounding for an obvious monitor command through the LLM `bash` tool path. RPC `bash` bypassed this path and is not a valid auto-background test.
7. ⏳ Test interactive dashboard behavior and shortcut ergonomics.
8. ⏳ Watch interaction with `pi-context-prune` batching: wake streams should not fragment batches or harm prompt-cache locality.
9. If promoted, update `pi-packages.json`, `README.md`, `pi-setup.sh` if config bootstrap is needed, `wiki/tools/pi-agent.md`, and `wiki/tools/pi-background-task-plugins.md` in the same logical unit.
