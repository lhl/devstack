...
**Usage stratifies by difficulty.** Frontier models keep the hardest tier — long-horizon autonomous work, novel reasoning, specialized enterprise offerings like the Fable class — where their capability lead is real and defensible for now. Open and custom models absorb the high-volume remainder at 1–10% of the cost [31]–[37]. The gateway (§8.4) is the mechanism that performs the split, enterprise by enterprise. One dynamic keeps the split unstable in the labs' favor: the hard tier is not standing still. If the EdgeBench trend holds — environment-learning speed doubling roughly every three months (§6) — the frontier tier keeps annexing work that looks impossible today, and the boundary between tiers reprices at every model release [53]–[55].
